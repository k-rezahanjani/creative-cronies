const PackageCards = ({ packageList }) => {
    return ( 
      <div className='d-flex flex-row justify-content-center'>
        {packageList.packages.map((card, index) => (
          <div key={index} className="card me-2 p-3" 
            style={{ width: '20rem', backgroundColor: index % 2 === 0 ? '#DBD8D3' : '#5B2E91', color: index % 2 === 0 ? 'black' : 'white'}}>
            <div className="card-body p-0">
              ...
              <div className='mt-1'>
                {card.options.map((option, optionIndex) => (
                  <p key={optionIndex} className='card-text'>{option}</p>
                ))}
              </div>
            </div>
          </div>
